...
describe("Webview Async Resolution", () => {
	...
	it("should queue messages until async resolution completes", async () => {
		...
		// Register the provider
		extensionHost.registerWebviewProvider("kilo-ssy.SidebarProvider", mockProvider)
		...
		if (vscode && vscode.window) {
			vscode.window.registerWebviewViewProvider("kilo-ssy.SidebarProvider", mockProvider)
		}
