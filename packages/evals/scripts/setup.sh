...
build_extension() {
  ...
  pnpm -w vsix -- --out ../bin/kilo-ssy-$(git rev-parse --short HEAD).vsix || exit 1
...
if ! code --list-extensions 2>/dev/null | grep -q "shengsuan-cloud.kilo-ssy"; then
  code --install-extension shengsuan-cloud.kilo-ssy &>/dev/null || exit 1
fi
