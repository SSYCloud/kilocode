	...
	switch (message.type) {
		...
		case "showFeedbackOptions": {
			...
			if (answer === githubIssuesText) {
				await vscode.env.openExternal(vscode.Uri.parse("https://github.com/SSYCloud/kilocode/issues"))
		...
		case "fetchProfileDataRequest":
			try {
				const { apiConfiguration } = await provider.getState()
				const token = apiConfiguration?.shengSuanYunXToken
				if (!token) {
					provider.log("shengSuanYun token not found in extension state.")
					provider.postMessageToWebview({
						type: "profileDataResponse",
						payload: { success: false, error: "shengSuanYun API token not configured." },
					})
					break
				}
				const header = {
		...
		case "fetchBalanceDataRequest": // New handler
			try {
				const { apiConfiguration } = await provider.getState()
				const kilocodeToken = apiConfiguration?.shengSuanYunXToken
