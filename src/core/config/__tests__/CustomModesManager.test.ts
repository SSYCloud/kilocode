...
describe("CustomModesManager", () => {
	...
	const mockRoomodes = `${path.sep}mock${path.sep}workspace${path.sep}.kilossymodes`
	...
	describe("getCustomModes", () => {
		it("should merge modes with .kilossymodes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .kilossymodes since it takes precedence
		...
		it("should handle missing .kilossymodes file", async () => {
		...
		it("should handle invalid JSON in .kilossymodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .kilossymodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .kilossymodes precedence", async () => {
			...
			// Should update global state with merged modes where .kilossymodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .kilossymodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .kilossymodes file when adding project-specific mode", async () => {
			...
			// Mock .kilossymodes to not exist initially
			...
			// Verify .kilossymodes was created with the project mode
			...
			// Verify the content written to .kilossymodes
