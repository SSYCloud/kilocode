...
export class McpHub {
		...
		try {
			...
			if (!result.success) {
				const errorMessages = result.error.errors
					.map((err) => `${err.path.join(".")}: ${err.message}`)
					.join("\n")
				console.error(t("mcp:errors.invalid_settings_validation", { errorMessages }))
  ...
  "mcpServers": {
	...
	private async initializeMcpServers(source: "global" | "project"): Promise<void> {
		try {
			...
			if (error instanceof SyntaxError) {
				...
				this.showErrorMessage(`初始化 ${source} MCP 服务失败`, error)
			}
		}
	}
	...
	private async getProjectMcpPath(): Promise<string | null> {
		...
		const projectMcpDir = path.join(workspaceFolder.uri.fsPath, ".kilossy")
